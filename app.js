...
function topicTotal(tp) { return tp.type === 'match' ? (tp.count || 10) : tp.type === 'vocab' ? tp.words.length : tp.questions.length; }
...
function renderDict(level) {
  ...
  document.getElementById('dictHead').innerHTML =
    ...
    '<button class="btn-shuffle dict-toex">' + t('toTopics') + '</button>';
  document.querySelector('.dict-toex').addEventListener('click', () => { location.hash = level; });
  ...
  const dictOrder = shuffle(DICT[level].slice());
  function draw(filter) {
    const f = trLower(filter || '');
    list.innerHTML = '';
    dictOrder.forEach(item => {
...
function renderTopics(level) {
  ...
  grid.classList.toggle('level-complete', allDone);
...
function show(r) {
  ...
  document.querySelector('#screen-quiz .shuffle-bar').style.display = (r.s === 'quiz' && (r.topic.type === 'vocab' || r.topic.type === 'match')) ? 'none' : '';
  if (r.s === 'quiz') {
...
function buildSections(mixed) {
  ...
  if (!curTopic.multi) {
    return [{ title: curTopic.secTitle[LANG], sub: curTopic.secSub[LANG], base: shuffle(qs.slice()) }];
  }
  const secs = [];
  qs.forEach(q => {
  ...
  secs.forEach(s => { s.base = shuffle(s.base); });
  return secs;
}
...
function buildDndCard(s, q, k, box) {
  ...
  function applyAnswer(oi) {
    ...
    finish();
    fb.innerHTML = (isOk ? 'Doğru! ' : 'Maalesef. ') + q.ok[LANG];
  ...
  cardFinishers.push(() => {
    ...
    finish();
    fb.innerHTML = t('timeUp') + ' ⏱️ ' + q.ok[LANG];
...
/* ── Вправа «Пари»: зʼєднай турецьке слово з перекладом (type: match) ── */
function renderMatch(append) {
  quizBuilt = true; streak = 0; clearTimer(); cardFinishers = [];
  ...
  const level = curTopic.level;
  const per = curTopic.pairs || 5;
  const count = curTopic.count || 10;
  const dict = DICT[level].slice();
  const weak = shuffle(dict.filter(x => wordStatus(x.w) < 2));
  const rest = shuffle(dict.filter(x => wordStatus(x.w) >= 2));
  const pool = shuffle(weak.concat(rest).slice(0, count));
  total = pool.length;
  ...
  document.getElementById('quizRestart').onclick = () => renderMatch();

  const head = document.createElement('div');
  head.className = 'section-head';
  head.innerHTML = '<h2>' + (curTopic.secTitle ? curTopic.secTitle[LANG] : 'Пари') + '</h2>' +
                   '<p>' + (curTopic.secSub ? (curTopic.secSub[LANG] || '') : '') + '</p>';
  quizMount.appendChild(head);

  const board = document.createElement('div');
  board.className = 'card match-card';
  quizMount.appendChild(board);

  let matched = 0, mistakes = 0, roundStart = 0;

  function updateBar() {
    document.getElementById('quizFill').style.width = Math.round(matched / total * 100) + '%';
    document.getElementById('scorePill').textContent = matched + ' / ' + total;
  }

  function renderRound() {
    const chunk = pool.slice(roundStart, roundStart + per);
    board.innerHTML = '<p class="match-hint">' +
      (curTopic.matchHint ? curTopic.matchHint[LANG] : 'Тапни турецьке слово, потім його переклад.') + '</p>';
    ...
    const left = shuffle(chunk.slice());
    const right = shuffle(chunk.slice());
    left.forEach(w => {
      const b = document.createElement('button');
      b.className = 'match-btn'; b.dataset.key = w.w; b.dataset.side = 'l';
      b.textContent = w.w; colL.appendChild(b);
    });
    right.forEach(w => {
      const b = document.createElement('button');
      b.className = 'match-btn'; b.dataset.key = w.w; b.dataset.side = 'r';
      b.textContent = w.tr[LANG] || w.tr.uk; colR.appendChild(b);
    });

    let sel = null, filled = 0;
    function onClick(btn) {
      ...
      if (sel.dataset.side === btn.dataset.side) { sel.classList.remove('sel'); sel = btn; btn.classList.add('sel'); return; }

      if (sel.dataset.key === btn.dataset.key) {
        const L = sel.dataset.side === 'l' ? sel : btn;
        const R = sel.dataset.side === 'r' ? sel : btn;
        L.classList.remove('sel'); L.classList.add('ok'); R.classList.add('ok');
        if (ttsOk) speak(L.dataset.key, 0.9);
        /* FLIP: плавне під'їжджання пари нагору й вирівнювання в рядок */
        const movers = [...colL.children, ...colR.children];
        const firstTop = movers.map(t => t.getBoundingClientRect().top);
        colL.insertBefore(L, colL.children[filled] || null);
        colR.insertBefore(R, colR.children[filled] || null);
        filled++;
        movers.forEach((t, i) => {
          const dy = firstTop[i] - t.getBoundingClientRect().top;
          if (!dy) return;
          ...
          requestAnimationFrame(() => {
            ...
            t.style.transform = '';
            setTimeout(() => t.classList.remove('flip'), 420);
          });
        });
        matched++; updateBar();
        sel = null;
        if (filled === chunk.length) {
          roundStart += per;
          setTimeout(roundStart >= total ? finishMatch : renderRound, 520);
        }
      } else {
        mistakes++;
        const a = sel, c = btn;
        a.classList.remove('sel'); a.classList.add('bad'); c.classList.add('bad');
        setTimeout(() => { a.classList.remove('bad'); c.classList.remove('bad'); }, 500);
        sel = null;
      }
    }
    [...colL.children, ...colR.children].forEach(btn => btn.addEventListener('click', () => onClick(btn)));
  }

  function finishMatch() {
    saveBest(curTopic.id, total);
    checkAllDone();
    const res = document.createElement('div');
    res.className = 'sec-result show';
    const verdict = mistakes === 0 ? 'Mükemmel! Без жодної помилки 🧿'
                  : mistakes <= 2 ? 'Çok iyi! Майже ідеально.'
                  : 'Güzel! Словник міцнішає.';
    res.innerHTML =
      '<div class="sec-score">' + total + ' / ' + total + ' ' + t('pairsWord') + '</div>' +
      '<p class="sec-text">' + verdict + (mistakes ? ' ' + t('mistakesWord') + ': ' + mistakes + '.' : '') + '</p>' +
      '<div class="sec-btns">' +
        '<button class="btn-restart m-again">' + t('again') + '</button>' +
        '<button class="btn-shuffle m-back">' + t('toTopics') + '</button>' +
      '</div>';
    quizMount.appendChild(res);
    celebrate(mistakes === 0 ? 200 : 120);
    if (mistakes === 0) ayYildiz();
    res.querySelector('.m-again').addEventListener('click', () => renderMatch());
    res.querySelector('.m-back').addEventListener('click', () => { location.hash = curTopic.level; });
    afterTopicDone(res);
  }
...
function renderVocab() {
  ...
  finishEl.classList.remove('show');
  sections = [];
  const words = shuffle(curTopic.words.slice());
  total = words.length;
...
function renderQuiz(mixed, timed, append) {
  if (curTopic.type === 'match') { renderMatch(append); return; }
...
function doQuizRestart() {
  if (curTopic.type === 'match') renderMatch();
  else if (curTopic.type === 'vocab') renderVocab();
