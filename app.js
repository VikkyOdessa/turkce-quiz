...
function renderMatch() {
  ...
  function renderRound() {
    ...
    for (let i = 0; i < chunk.length; i++) {
      ...
      lb.className = 'match-btn'; lb.dataset.key = left[i].w; lb.dataset.side = 'l';
      lb.textContent = left[i].w;
      const rb = document.createElement('button');
      rb.className = 'match-btn'; rb.dataset.key = right[i].w; rb.dataset.side = 'r';
      rb.textContent = right[i].tr[LANG] || right[i].tr.uk;
      wrap.appendChild(lb); wrap.appendChild(rb);
    }
    board.appendChild(wrap);

    let sel = null;
    wrap.querySelectorAll('.match-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        if (btn.classList.contains('gone') || btn.classList.contains('ok')) return;
        ...
        if (sel.dataset.key === btn.dataset.key) {
          const a = sel, c = btn;
          a.classList.remove('sel'); a.classList.add('ok'); c.classList.add('ok');
          if (ttsOk) speak(a.dataset.key, 0.9);
          setTimeout(() => { a.classList.add('gone'); c.classList.add('gone'); }, 350);
          matched++; updateBar();
          sel = null;
          const done = [...wrap.querySelectorAll('.match-btn')].every(x => x.classList.contains('ok'));
          if (done) {
            roundStart += per;
            setTimeout(roundStart >= total ? finishMatch : renderRound, 480);
          }
        } else {
          mistakes++;
          const a = sel, c = btn;
